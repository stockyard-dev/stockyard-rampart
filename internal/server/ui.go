...
async function load(){var r=await fetch(A+'/block_rules').then(function(r){return r.json()});items=r.block_rules||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.cidr||'').toLowerCase().includes(q)||(i.reason||'').toLowerCase().includes(q)||(i.source||'').toLowerCase().includes(q)||(i.enabled||'').toLowerCase().includes(q)});
if(!f.length){document.getElementById('list').innerHTML='<div class="empty">No items found.</div>';return;}
var h='';f.forEach(function(i){
h+='<div class="item"><div class="item-top"><div style="flex:1"><div class="item-title">'+esc(i.cidr)+'</div>';
if(i.reason)h+='<div class="item-sub">'+esc(i.reason)+'</div>';
...
if(i.hit_count)h+='<span>'+esc(i.hit_count)+'</span>';
if(i.expires_at)h+='<span>'+esc(i.expires_at)+'</span>';
if(i.last_hit_at)h+='<span>'+esc(i.last_hit_at)+'</span>';
h+='<span>'+ft(i.created_at)+'</span></div>';

h+='</div>';});document.getElementById('list').innerHTML=h;}
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/block_rules/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{cidr:"",reason:"",source:"",enabled:"",hit_count:"",expires_at:"",last_hit_at:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' RAMPART</h2>';
h+='<div class="fr"><label>C I D R *</label><input id="f-cidr" value="'+esc(i.cidr)+'"></div>';
h+='<div class="fr"><label>Reason</label><input id="f-reason" value="'+esc(i.reason)+'"></div>';
...
h+='<div class="fr"><label>Hit Count</label><input id="f-hit-count" value="'+esc(i.hit_count)+'"></div>';
h+='<div class="fr"><label>Expires At</label><input id="f-expires-at" value="'+esc(i.expires_at)+'"></div>';
h+='<div class="fr"><label>Last Hit At</label><input id="f-last-hit-at" value="'+esc(i.last_hit_at)+'"></div>';
...
async function submit(){var body={cidr:document.getElementById('f-cidr').value.trim(),reason:document.getElementById('f-reason').value.trim(),source:document.getElementById('f-source').value.trim(),enabled:document.getElementById('f-enabled').value.trim(),hit_count:document.getElementById('f-hit-count').value.trim(),expires_at:document.getElementById('f-expires-at').value.trim(),last_hit_at:document.getElementById('f-last-hit-at').value.trim()};
if(editId){await fetch(A+'/block_rules/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/block_rules',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
