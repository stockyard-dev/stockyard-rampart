...
s.mux.HandleFunc("GET /api/block_rules",s.list);s.mux.HandleFunc("POST /api/block_rules",s.create);s.mux.HandleFunc("GET /api/block_rules/{id}",s.get);s.mux.HandleFunc("DELETE /api/block_rules/{id}",s.del)
...
func(s *Server)list(w http.ResponseWriter,r *http.Request){wj(w,200,map[string]any{"block_rules":oe(s.db.List())})}
func(s *Server)create(w http.ResponseWriter,r *http.Request){var e store.BlockRule;json.NewDecoder(r.Body).Decode(&e);if e.CIDR==""{we(w,400,"cidr required");return};s.db.Create(&e);wj(w,201,s.db.Get(e.ID))}
...
func(s *Server)stats(w http.ResponseWriter,r *http.Request){wj(w,200,map[string]int{"block_rules":s.db.Count()})}
