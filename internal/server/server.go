...
func(s *Server)create(w http.ResponseWriter,r *http.Request){if s.limits.MaxItems>0{if s.db.Count()>=s.limits.MaxItems{we(w,402,"Free tier limit reached. Upgrade at https://stockyard.dev/rampart/");return}};var e store.BlockRule;json.NewDecoder(r.Body).Decode(&e);if e.CIDR==""{we(w,400,"cidr required");return};s.db.Create(&e);wj(w,201,s.db.Get(e.ID))}
