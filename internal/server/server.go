package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-rampart/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-rampart"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/entries",s.handleList)
    s.mux.HandleFunc("POST /api/entries",s.handleAdd)
    s.mux.HandleFunc("DELETE /api/entries/{id}",s.handleRemove)
    s.mux.HandleFunc("GET /api/lookup/{ip}",s.handleLookup)
